...
<body>
	...
	<form action="<%=path %>/admin/suffererUpdateOk.do" id="form" method="post">
	<table class="table table-bordered" style="width:400px" align="center">
		...
		<tr>
			<td width="50%" align="right" bgcolor="#f1f1f1">所属医生：</td>
			<td width="50%">
				<select name="doc_id" id="doc_id" class="span1-1">
					<option value="">--请选择--</option>
					<c:forEach items="${requestScope.list}" var="doc">
					<option ${requestScope.sufferer.doc_id==doc.id?'selected=selected':''} value="${doc.id}">${doc.doc_name}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td width="50%">
				<input type="text" name="suf_card" value="${requestScope.sufferer.suf_card}" maxlength="20" id="suf_card" class="span1-1" />
			</td>
		</tr>
		<tr>
